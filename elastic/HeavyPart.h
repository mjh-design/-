...
public:
	...
	int insert(uint8_t *key, uint8_t *swap_key, uint32_t &swap_val, uint32_t f = 1)
	{
		...
		__m128i low_part = _mm_castps_si128(_mm256_extractf128_ps(results, 0)); //_mm_castps_si128仅用于类型转换将_m128转换为_m128i
		...
		int min_counter_val = _mm_cvtsi128_si32(min4); // 取低32bit
